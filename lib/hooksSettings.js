 ...
 * The server passes them inline with `-c hooks...` and checks Codex's persisted
 * trust before launching a task. Review uses the exact same definitions.
...
function buildHookArgs(nodeBin = process.execPath) {
  ...
  const entries = [
    ...
    ['PostToolUse', commandFor('PostToolUse')],
