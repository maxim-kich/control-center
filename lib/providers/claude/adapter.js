...
function buildLaunchArgs({ kind, sessionId, parentSessionId, name, prompt, settingsPath, mcpConfigPath, model, effort, mode, skipPermissions, ultracode }) {
  ...
  const modelInfo = catalog.snapshot('claude').models.find((m) => m.id === model);
  if (effort && (!modelInfo?.efforts || modelInfo.efforts.length)) args.push('--effort', normalizeEffort(effort, model));
...
function normalizeEffort(effort, model) {
  const info = catalog.snapshot('claude').models.find((m) => m.id === model);
...
const adapter = {
  ...
  defaultModel() {
    return catalog.snapshot('claude').defaultModel;
  },

  models() {
    return catalog.snapshot('claude').models;
  },
  ...
  normalizeTaskSettings(task) {
    ...
    return {
      ...
      effort: catalog.snapshot('claude').models.find((m) => m.id === task.model)?.efforts?.includes(task.effort)
        || ['low', 'medium', 'high', 'xhigh', 'max'].includes(task.effort) ? task.effort : 'medium',
      mode,
  ...
  setupExtension({ extension, integration, project, action } = {}) {
    ...
    if (action === 'cleanup') return { ok: true, skipped: true };
    const detected = adapter.detect();
    if (!detected.installed || !detected.connected) {
      return { ok: true, skipped: true, required: false, provider: adapter.id, status: detected.status };
    }
    const projectPath = resolveProjectPath(project && project.path ? project.path : project);
    if (!safeIsDir(projectPath)) return { ok: false, error: `project path does not exist: ${projectPath}` };
    return {
      ok: true,
      provider: adapter.id,
      graphifyMcp: adapter.supports.graphifyMcp,
      mcpCommand: graphifyMcpBin(),
      graphPath: graphJsonFor(projectPath),
    };
  },
