...
const { watchInterruptions } = require('./interruption');
...
const adapter = {
  ...
  defaultModel() {
    return catalog.snapshot('codex').defaultModel;
  },

  models() {
    return catalog.snapshot('codex').models;
  },
  ...
  normalizeTaskSettings(task) {
    ...
    return {
      ...
      effort: codex.normalizeEffort(task.effort, task.model),
      mode,
  ...
  onLaunch({ task, kind, sessionId, parentSessionId, launch, runner, db }) {
    watchInterruptions({ taskId: task.id, runner, db, findTranscript: codex.findTranscriptPath,
