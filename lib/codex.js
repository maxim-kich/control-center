...
function codexVersion() {
  try {
    return execFileSync(CODEX_BIN, ['--version'], {
      encoding: 'utf8',
      timeout: 1000,
      // A CLI that ignores SIGTERM must not keep the synchronous probe blocked.
      killSignal: 'SIGKILL',
...
function normalizeModel(model) {
  if (!model || /^claude-/i.test(String(model))) return require('./modelCatalog').catalog.snapshot('codex').defaultModel;
  return String(model).trim();
}

function normalizeEffort(effort, model) {
  const info = require('./modelCatalog').catalog.snapshot('codex').models.find((m) => m.id === model);
...
function buildArgs({
  ...
  const normalizedEffort = normalizeEffort(effort, normalizedModel);
  ...
  const modelInfo = require('./modelCatalog').catalog.snapshot('codex').models.find((m) => m.id === normalizedModel);
  if (!modelInfo?.efforts || modelInfo.efforts.length) args.push('-c', `model_reasoning_effort=${JSON.stringify(normalizedEffort)}`);
  args.push(...hookArgs);
