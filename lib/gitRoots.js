...
function resolveProjectGit(projectPath) {
  ...
  return classifyProjectGit(resolvedProjectPath, ownGit, discovered);
}

function classifyProjectGit(resolvedProjectPath, ownGit, discovered) {
...
const pendingProjectGit = new Map();

function clearProjectGitCache() {
  // Detached probes may finish, but must not repopulate an invalidated cache.
  pendingProjectGit.clear();
  projectGitCache.clear();
}

async function projectGitApiFieldsAsync(projectPath) {
  ...
  const ownGit = hasProjectGit(key);
  let pending = pendingProjectGit.get(key);
  if (!pending || pending.ownGit !== ownGit) {
    pending = { ownGit };
    pending.promise = new Promise((resolve) => {
      ...
      const info = classifyProjectGit(key, ownGit, discovered);
      const fields = {
      ...
      if (pendingProjectGit.get(key) === pending) {
        pendingProjectGit.delete(key);
        if (hasProjectGit(key) === ownGit) {
          projectGitCache.set(key, { fields, expiresAt: Date.now() + PROJECT_GIT_CACHE_MS });
        }
      }
      return fields;
    });
    pendingProjectGit.set(key, pending);
  }
