...
function scanExtensions(extensionsDir) {
  ...
  for (const dir of listExtensionDirs(extensionsDir)) {
    ...
    try {
      read = readManifest(dir);
    } catch (error) {
      manifestReadError = error;
      read = { manifest: {}, file: path.join(dir, 'extension.json') };
    }
    if (!read) continue;
    let extension;
    try {
...
function publicExtension(extension) {
  const enabled = extension.enabledByUser !== false && extension.errors.length === 0;
  return {
    id: extension.id,
    ...
    description: extension.description,
    enabled,
    enabledByUser: extension.enabledByUser !== false,
    errors: extension.errors,
    permissions: extension.permissions,
    ownership: extension.ownership,
    frontend: enabled ? extension.frontend : { scripts: [], styles: [] },
    contributes: enabled ? extension.contributes : Object.fromEntries(Object.keys(CONTRIBUTION_DEFS).map((key) => [key, []])),
    settingsPanels: enabled ? extension.settingsPanels : [],
    taskDetailSections: enabled ? extension.taskDetailSections : [],
    projectActions: enabled ? extension.projectActions : [],
    routes: enabled ? extension.routes.map((route) => ({ ...route, mount: `/api/extensions/${extension.id}/${route.path}`.replace(/\/$/, '') })) : [],
    migrations: extension.migrations,
    hooks: enabled ? extension.hooks : [],
  };
}
...
function loadExtensionBackend(app, extension, context, lifecycle, platform) {
  ...
  try {
    ...
    if (mod && typeof mod.register === 'function') {
      ...
      if (app && router) app.use(`/api/extensions/${extension.id}`, router);
      extension.backendRouter = router;
      extension.routesMounted = !!app;
    }
    extension.backendLoaded = true;
  } catch (e) {
...
function loadExtensions(opts = {}) {
  ...
  for (const extension of scanned.extensions) {
    ...
    if (app && extension.publicDir && extension.enabledByUser && extension.errors.length === 0) app.use(`/extensions/${extension.id}`, express.static(extension.publicDir));
  ...
  return {
    ...
    activate(extensionId) {
      let extension = this.extensions.find((item) => item.id === extensionId);
      if (!extension) {
        extension = scanExtensions(extensionsDir).extensions.find((item) => item.id === extensionId);
        if (!extension) throw new Error(`extension is not installed: ${extensionId}`);
        this.extensions.push(extension);
      }
      extension.enabledByUser = opts.platform.isEnabled(extensionId);
      const activationApp = opts.activationApp || app;
      if (!extension.backendLoaded) loadExtensionBackend(activationApp, extension, opts.context || {}, lifecycle, opts.platform);
      if (!extension.routesMounted && extension.backendRouter && activationApp) {
        activationApp.use(`/api/extensions/${extension.id}`, extension.backendRouter);
        extension.routesMounted = true;
      }
      if (extension.errors.length) throw new Error(extension.errors.join('; '));
      return extension;
    },
    async disable(extensionId) {
      const extension = this.extensions.find((item) => item.id === extensionId);
      if (!extension || !opts.platform) throw new Error(`extension is not installed: ${extensionId}`);
      await this.lifecycle.notify('app.stopping', { reason: 'extension-disabled' }, { extensionId });
      const status = opts.platform.disable(extensionId);
      extension.enabledByUser = false;
      return status;
    },
  };
}
