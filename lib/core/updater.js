...
async function runBundledIntegrationMigration(opts = {}) {
  ...
  try {
    ...
    if (existing && existing.status === 'completed' && existing.version === BUNDLED_INTEGRATION_MIGRATION_VERSION) {
      return { ok: true, alreadyCompleted: true, plan: readMetaJson(handle, BUNDLED_INTEGRATION_PLAN_KEY, plan), ledger: existing };
    }
    ...
    await platform.runHealthChecks();
...
async function updateGitCheckout(opts = {}) {
  ...
  try {
    ...
    npmInstall(root);
    const appliedBundledMigration = await runBundledIntegrationMigration({ ...opts, root, dbPath, dryRun: false });
    ...
    return { ok: true, target, before, backup, migration, bundledMigration: appliedBundledMigration };
  } catch (e) {
    try {
