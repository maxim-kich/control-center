...
function parsePorcelainZ(output) {
  ...
  for (let i = 0; i < entries.length; i += 1) {
    ...
    if ((status[0] === 'R' || status[0] === 'C') && entries[i + 1]) {
      originalPath = normalizeRel(entries[i + 1]);
      i += 1;
    }
    file = normalizeRel(file);
    if (file) changes.push({ status, path: file, ...(originalPath ? { originalPath } : {}) });
  }
  return changes;
}
...
function imageOwnedChanges(root) {
  ...
  const changes = parsePorcelainZ(raw).filter((change) => {
    const paths = [change.path, change.originalPath].filter(Boolean);
    return paths.some((file) => matchesAny(file, manifest.imageOwned) && !matchesAny(file, manifest.generated));
  });
  return { ok: changes.length === 0, git: true, changes };
}

function safeCheckoutPath(root, relativePath) {
  const absoluteRoot = path.resolve(root);
  const absolutePath = path.resolve(absoluteRoot, relativePath);
  if (absolutePath === absoluteRoot || !absolutePath.startsWith(`${absoluteRoot}${path.sep}`)) {
    throw new Error(`Refusing to overwrite path outside the application checkout: ${relativePath}`);
  }
  return absolutePath;
}

function overwriteImageOwnedChanges(root, opts = {}) {
  const initial = imageOwnedChanges(root);
  if (!initial.git || initial.ok || opts.dryRun) return initial;

  const touched = [...new Set(initial.changes.flatMap((change) =>
    [change.path, change.originalPath].filter(Boolean),
  ))];
  if (touched.length) runGit(root, ['reset', '-q', 'HEAD', '--', ...touched]);

  const manifest = readManifest(root);
  const raw = runGit(root, ['status', '--porcelain=v1', '-z', '--untracked-files=all'], { timeoutMs: 10000, trim: false });
  const remaining = parsePorcelainZ(raw).filter((change) =>
    matchesAny(change.path, manifest.imageOwned) && !matchesAny(change.path, manifest.generated),
  );
  const tracked = remaining.filter((change) => change.status !== '??').map((change) => change.path);
  const untracked = remaining.filter((change) => change.status === '??').map((change) => change.path);

  if (tracked.length) runGit(root, ['checkout', 'HEAD', '--', ...tracked]);
  for (const relativePath of untracked) {
    fs.rmSync(safeCheckoutPath(root, relativePath), { recursive: true, force: true });
  }

  const final = imageOwnedChanges(root);
  if (!final.ok) {
    const err = new Error('Could not overwrite all image-owned files before update.');
    err.code = 'DIRTY_IMAGE';
    err.changes = final.changes;
    throw err;
  }
  return initial;
}
...
function snapshotSqlite(dbPath, destination) {
  const handle = new Database(dbPath, { readonly: true, fileMustExist: true });
  try {
    // Raw file copies close a descriptor behind SQLite's back, releasing
    // process-wide POSIX locks. Keep all live-database I/O inside SQLite.
    handle.prepare('VACUUM INTO ?').run(destination);
  } finally {
    handle.close();
  }
}
...
function backupInstance(opts = {}) {
  ...
  if (dbPath && fs.existsSync(dbPath)) {
    const destination = path.join(outDir, path.basename(dbPath));
    snapshotSqlite(dbPath, destination);
...
function createBundledIntegrationMigrationPlan(opts = {}) {
  ...
  for (const item of BUNDLED_INTEGRATIONS) {
    ...
    const targetOwner = item.domain !== 'graphify' && domainUsage.used ? item.extensionId : 'legacy';
    targets[item.domain] = {
      domain: item.domain,
...
async function runBundledIntegrationMigration(opts = {}) {
  ...
  try {
    ...
    // Graphify is opt-in, even when repairing a plan saved by an older release.
    if (plan.targets.graphify) plan.targets.graphify.targetOwner = 'legacy';
    for (const bundle of plan.bundles) {
      if (bundle.id === 'graphify' || bundle.install === false) continue;
...
async function updateGitCheckout(opts = {}) {
  ...
  const imageChanges = overwriteImageOwnedChanges(root, { dryRun: true });
  ...
  if (opts.dryRun) {
    return { ok: true, dryRun: true, target, before, backup, migration, bundledMigration, imageChanges: imageChanges.changes, extensionConflicts: extensions.conflicts };
  }

  try {
    overwriteImageOwnedChanges(root);
    ...
    return { ok: true, target, before, backup, migration, bundledMigration, imageChanges: imageChanges.changes, startupMigration: startupContext };
  } catch (e) {
    try {
...
function rollbackGitCheckout(opts = {}) {
  ...
  if (opts.dryRun) return { ok: true, dryRun: true, target, before, backup, imageChanges: imageChanges.changes, extensionConflicts: extensions.conflicts };
  ...
  return { ok: true, target, before, backup, imageChanges: imageChanges.changes };
}
