...
class ExtensionLifecycle {
  ...
  handlersFor(name) {
    return this.handlers.filter((entry) => entry.name === name && entry.extension.errors.length === 0);
  }
  ...
  async notify(name, context) {
    const results = [];
    for (const entry of this.handlersFor(name)) results.push(await this.invoke(entry, context));
