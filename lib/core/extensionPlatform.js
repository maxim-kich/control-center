    ...
    if (granted.has('git:read') || granted.has('git:write')) {
      capabilities.git = Object.freeze({
        ...
        init: async (projectPath, opts = {}) => {
          ...
          // Explicit initialization creates a project-local repository even inside a parent repo.
          // Other Git operations still require the project to own its repository.
      ...
      capabilities.providers = Object.freeze({
        setup: async (providerId, project, opts = {}) => {
          ...
          if (!detected.installed) {
            // Graphify shares one graph across the available agents. An absent
            // agent is not a failed integration (including older installed bundles).
            if (opts.integration === 'graphify' || extension.id === 'graphify') {
              return { ok: true, skipped: true, required: false, provider: providerId, status: 'missing' };
            }
            throw new Error(`${providerId} CLI not found`);
          }
