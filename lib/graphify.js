...
function shouldIgnoreWatchFile(filename) {
  ...
  if (normalized === 'AGENTS.md' || normalized === 'CLAUDE.md') return true;
...
class GraphifyManager {
  constructor(db, opts) {
    ...
    this.providers = Array.isArray(opts.providers) && opts.providers.length
      ? opts.providers.map((item) => String(item || '').trim()).filter(Boolean)
      : String(process.env.CC_GRAPHIFY_PROVIDERS || 'codex,claude').split(',').map((item) => item.trim()).filter(Boolean);
    ...
    this.providerSetup = typeof opts.providerSetup === 'function' ? opts.providerSetup : null;
    this.providerCleanup = typeof opts.providerCleanup === 'function' ? opts.providerCleanup : null;
  ...
  async drain() {
    ...
    try {
      ...
      // Provider setup and managed processes can reject. Keep optional graph
      // work from becoming an unhandled rejection that terminates the server.
      this.mark(project.id, {
        graphify_status: this.projectEnabled(this.db.getProject(project.id)) ? 'error' : 'disabled',
        graphify_last_finished_at: this.db.now(),
        graphify_last_error: `Graphify failed: ${error && error.message ? error.message : String(error)}`,
      });
    } finally {
  ...
  async runProject(project, reason) {
    ...
    const setupNeeded = this.alwaysInstall
      ...
      || reason === 'bootstrap';
    if (setupNeeded && !this.providerSetup) {
    ...
    if (setupNeeded && this.providerSetup) {
      for (const provider of this.providers) {
        const setup = await this.providerSetup(provider, project, {
          reason,
          setupNeeded,
          graphifyBin: this.bin,
        ...
        if (setup && setup.ok === false && setup.required !== false) {
          this.mark(project.id, {
            graphify_status: 'error',
            graphify_last_finished_at: this.db.now(),
            graphify_last_error: setup.error || setup.detail || `Graphify provider setup failed for ${provider}`,
          });
          return;
        }
      }
    ...
    let hookError = null;
    const git = resolveProjectGit(project.path);
    if (git.hasOwnGit) {
      ...
      hookStatus = 'parent_repo';
      hookError = git.warning;
    } else {
      hookStatus = 'not_git';
    }
  ...
  async uninstallProjectGraphify(project) {
    ...
    if (this.providerCleanup) {
      for (const provider of this.providers) {
        await this.providerCleanup(provider, project, {
          graphifyBin: this.bin,
          timeoutMs: this.setupTimeoutMs,
        });
      }
  ...
  runGraphify(cwd, args, opts) {
    const label = `graphify-${path.basename(cwd || 'project')}-${(args || []).join('-').replace(/[^A-Za-z0-9_.-]+/g, '-').slice(0, 80) || 'run'}`;
    return this.processRunner(this.bin, args, { cwd, name: label, ...opts });
  }
